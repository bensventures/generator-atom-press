...
module.exports = generators.Base.extend({
  ...
  default : function (){
    this.themeroot = this._.slugify(this.themename);
    //this.mkdir(this.themeroot);
  },
  writing : function (){
    this.log.info('Downloading & extracting ' + chalk.yellow('_s'));

    this.remote('Automattic', '_s', this._baseThemeReady.bind(this));
  },

  _baseThemeReady : function (err, remote){
    this.mkdir(this.themeroot);

    remote.directory('.', this.themeroot);

    this._findandreplace(this.themeroot);
    this.log.ok('Done replacing string ' + chalk.yellow('_s'));

    this.log(chalk.yellow('Creating dev folders and files'));
    
    //this.mkdir(this.themeroot + '/build');

    this.copy('_main.scss', this.themeroot + '/css/sass/main.scss');
    this.copy('_theme.js', this.themeroot + '/js/theme.js');
    this.copy('_package.json', this.themeroot + '/build/package.json');
    this.copy('_bower.json', this.themeroot + '/build/bower.json');
    this.copy('Gruntfile.js', this.themeroot + '/build/Gruntfile.js');
    this.copy('_gitignore', this.themeroot + '/.gitignore');
  ...
  _findandreplace : function(dir) {
    ...
    files.forEach(function (file) {
      ...
      if (stat.isFile() && (path.extname(file) == '.php' || path.extname(file) == '.css')) {
        ...
        self._findandreplace.call(self, file);
      }
    });
  }
});